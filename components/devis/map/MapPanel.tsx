...
export function MapPanel({
  ...
  return (
    <div className="relative h-full w-full overflow-hidden bg-cloud map-grid">
      ...
      <div
        aria-hidden
        className="pointer-events-none absolute -left-16 top-10 h-72 w-72 rounded-full bg-aller/20 blur-3xl"
      />
      <div
        aria-hidden
        className="pointer-events-none absolute -right-10 bottom-0 h-80 w-80 rounded-full bg-lime/15 blur-3xl"
      />

      {/* Badge RSE (haut gauche) — glassmorphism */}
      ...
      {/* Badge distance / durée (haut droite) */}
      {(itineraire.distanceKm || itineraire.dureeConduite) && (
      ...
      {/* Aide à la lecture */}
      <p className="absolute bottom-5 left-1/2 z-10 -translate-x-1/2 rounded-full bg-night/80 px-3 py-1.5 text-[11px] font-medium text-white backdrop-blur-sm">
