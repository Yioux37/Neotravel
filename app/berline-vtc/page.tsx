        ...
        <section className="relative bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-slate-900 via-[#0B0F19] to-[#0B0F19] pt-12 pb-20 border-b border-slate-800/60">
          <div className="mx-auto max-w-6xl px-6">
            <nav className="text-xs text-slate-500 mb-6 flex items-center gap-2">
              <Link href="/" className="hover:text-slate-300 transition-colors">Accueil</Link><span>»</span><span className="text-slate-300">Berline VTC</span>
            </nav>
            ...
            <div className="max-w-3xl">
              <h1 className="text-4xl md:text-5xl font-bold tracking-tight text-white mb-4 font-display">Berline VTC</h1>
              <p className="text-lg text-slate-400 leading-relaxed mb-6">Le transport premium pour petits groupes et dirigeants. Luxe, discrétion et service sur-mesure pour vos transferts VIP.</p>
            </div>
            <div className="flex flex-wrap gap-3 mb-8">
              {["1 à 7 places VIP", "Chauffeur premium", "Transferts VIP"].map((tag) => (
        ...
        <section className="mx-auto max-w-6xl px-6 py-16 grid grid-cols-1 lg:grid-cols-3 gap-12 items-start">
          <div className="lg:col-span-2 space-y-8">
            <div>
              ...
              <h2 className="text-2xl md:text-3xl font-bold text-white mb-4">Quand choisir une berline ou van VIP ?</h2>
              <div className="text-slate-400 space-y-4 leading-relaxed">
                <p>Privilégiez ce service pour des délégations officielles, des transferts d'artistes, la direction d'entreprise ou des clients très importants (VIP).</p>
                <p>L'accent est mis sur la discrétion du chauffeur, le confort haut de gamme du véhicule et la flexibilité totale de votre agenda.</p>
              </div>
            </div>
          </div>
          <div className="rounded-2xl bg-gradient-to-b from-slate-900 to-slate-950 p-6 border border-slate-800 sticky top-6">
            ...
            <div className="space-y-4 mb-6 mt-6">
              {[
                { label: "Capacité", val: "1 à 7 places VIP" },
                { label: "Idéal pour", val: "Dirigeants, VIP, transferts premium" },
                { label: "Trajets", val: "Urbain premium, navettes aéroports" },
                { label: "Bagages", val: "Coffre adapté selon modèle" }
              ].map((spec, idx) => (
