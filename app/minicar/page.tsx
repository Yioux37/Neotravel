        ...
        <section className="relative bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-slate-900 via-[#0B0F19] to-[#0B0F19] pt-12 pb-20 border-b border-slate-800/60">
          <div className="mx-auto max-w-6xl px-6">
            <nav className="text-xs text-slate-500 mb-6 flex items-center gap-2">
              <Link href="/" className="hover:text-slate-300 transition-colors">Accueil</Link><span>»</span><span className="text-slate-300">Minicar</span>
            </nav>
            ...
            <div className="max-w-3xl">
              <h1 className="text-4xl md:text-5xl font-bold tracking-tight text-white mb-4 font-display">Minicar</h1>
              <p className="text-lg text-slate-400 leading-relaxed mb-6">Le compromis parfait entre la maniabilité d'un minibus et le confort d'un grand autocar. Idéal pour les excursions scolaires ou les sorties d'associations de taille moyenne.</p>
            </div>
            <div className="flex flex-wrap gap-3 mb-8">
              {["25 à 35 places", "Chauffeur inclus", "Idéal excursions"].map((tag) => (
            ...
            <div className="flex flex-wrap gap-4">
              ...
              <a href="#comparatif" className="rounded-full bg-slate-900 hover:bg-slate-800 px-6 py-3 text-sm font-medium text-slate-300 border border-slate-800 transition-all">Comparer la flotte</a>
            </div>
          </div>
        </section>
        ...
        <section className="mx-auto max-w-6xl px-6 py-16 grid grid-cols-1 lg:grid-cols-3 gap-12 items-start">
          <div className="lg:col-span-2 space-y-8">
            <div>
              ...
              <h2 className="text-2xl md:text-3xl font-bold text-white mb-4">Quand choisir un minicar ?</h2>
              <div className="text-slate-400 space-y-4 leading-relaxed">
                <p>Choisissez un minicar lorsque votre groupe dépasse les 25 personnes mais que vous n'avez pas besoin de l'encombrement d'un autocar standard de 50 places.</p>
                <p>Il offre un excellent compromis entre capacité, maniabilité sur route et budget, parfait pour des excursions régionales.</p>
              </div>
            </div>
          </div>
          <div className="rounded-2xl bg-gradient-to-b from-slate-900 to-slate-950 p-6 border border-slate-800 sticky top-6">
            ...
            <div className="space-y-4 mb-6 mt-6">
              {[
                { label: "Capacité", val: "25 à 35 places" },
                { label: "Idéal pour", val: "Excursions, groupes loisirs, scolaires" },
                { label: "Trajets", val: "Moyenne distance, excursions régionales" },
                { label: "Bagages", val: "Soutes de taille moyenne" }
              ].map((spec, idx) => (
